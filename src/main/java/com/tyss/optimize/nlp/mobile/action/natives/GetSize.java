...
@Slf4j
@Component(value =  "MOB_GetSize")
public class GetSize implements Nlp {
	...
	public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
		...
		try {
			...
			log.info("Getting size of " + elementName + " " + elementType);
			...
			nlpResponseModel.setMessage(modifiedPassMessage.replace("*returnValue*", String.valueOf(size)));
			nlpResponseModel.setStatus(CommonConstants.pass);
			nlpResponseModel.getAttributes().put("size",size);
		} catch(Throwable e) {
         	log.error("NLP_EXCEPTION in GetSize ", e);
	...
	public StringBuilder getTestCode() throws NlpException {
		...
		sb.append("Dimension size = element.getSize();\n");
