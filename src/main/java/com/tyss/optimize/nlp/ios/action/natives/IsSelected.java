...
@Slf4j
@Component(value = "IOS_IsSelected")
public class IsSelected implements Nlp {
    @Override
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            log.info("Getting the selected status of " + elementName + " " + elementType);
            selected = element.isSelected();
            log.info(elementName + " " + elementType + " is selected");
            nlpResponseModel.setMessage(modifiedPassMessage);
            nlpResponseModel.setStatus(CommonConstants.pass);
            nlpResponseModel.getAttributes().put("selected", selected);
        } catch (Throwable e) {
            log.error("NLP_EXCEPTION in IsSelected ", e);
