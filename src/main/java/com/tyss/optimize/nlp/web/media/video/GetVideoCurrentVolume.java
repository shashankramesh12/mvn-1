...
@Slf4j
@Component(value = "GetVideoCurrentVolume")
public class GetVideoCurrentVolume implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            Long currentVolume = (Long) ((JavascriptExecutor) driver).executeScript("return arguments[0].volume;", element);
            passMessage = passMessage.replace("*returnValue*", currentVolume.toString());
            nlpResponseModel.getAttributes().put("volume", (double)currentVolume);
            nlpResponseModel.setStatus(CommonConstants.pass);
            nlpResponseModel.setMessage(passMessage);
        } catch (Exception exception){
            log.error("NLP_EXCEPTION in GetVideoCurrentVolume ", exception);
