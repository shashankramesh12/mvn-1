...
@Slf4j
@Component(value = "GetNumberOfBrokenImages")
public class GetNumberOfBrokenImages implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            passMessage = passMessage.replace("*returnValue*", String.valueOf(brokenImages.size()));
            nlpResponseModel.getAttributes().put("brokenImages", Long.valueOf(brokenImages.size()));
            nlpResponseModel.setStatus(CommonConstants.pass);
            nlpResponseModel.setMessage(passMessage);
        } catch (Exception exception){
            log.error("NLP_EXCEPTION in GetNumberOfBrokenImages ", exception);
