...
@Slf4j
@Component(value = "VerifyIfLinksAreBroken")
public class VerifyIfLinksAreBroken implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            if (allLinks.size() == brokenLinks.size()){
                log.info("All links " + brokenLinks.toString() + " are broken");
                modifiedPassMessage = passMessage.replace("*returnValue*", brokenLinks.toString());
                nlpResponseModel.getAttributes().put("returnValue", brokenLinks);
                nlpResponseModel.setMessage(modifiedPassMessage);
                nlpResponseModel.setStatus(CommonConstants.pass);
            }
            else {
                log.error("Failed to verify if links " + unBrokenLinks + " are broken");
                nlpResponseModel.setMessage(modifiedFailMessage);
                nlpResponseModel.setStatus(CommonConstants.fail);
            }
        } catch (Exception exception){
            log.error("NLP_EXCEPTION in VerifyIfLinksAreBroken ", exception);
