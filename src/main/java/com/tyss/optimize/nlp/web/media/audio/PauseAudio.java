...
@Slf4j
@Component(value = "PauseAudio")
public class PauseAudio implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            ((JavascriptExecutor) driver).executeScript("arguments[0].pause();", element);
            Boolean paused =  (Boolean) ((JavascriptExecutor) driver).executeScript("return arguments[0].paused;", element);
            if(paused){
                nlpResponseModel.setStatus(CommonConstants.pass);
                nlpResponseModel.setMessage(passMessage);
                nlpResponseModel.getAttributes().put("audioPause", true);
            } else {
                nlpResponseModel.setStatus(CommonConstants.fail);
                nlpResponseModel.setMessage(modifiedFailMessage);
                nlpResponseModel.getAttributes().put("audioPause", false);
            }
            nlpResponseModel.setStatus(CommonConstants.pass);
        } catch (Exception exception){
            log.error("NLP_EXCEPTION in PauseAudio ", exception);
