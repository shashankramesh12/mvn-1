...
@Slf4j
@Component(value = "WaitTillElementVisible")
public class WaitTillElementVisible implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            log.info("Waiting for element " + elementName + " " + elementType + " to be visible");
            WebDriverWait wait = new WebDriverWait(driver, explicitTimeOut);
            wait.until(ExpectedConditions.visibilityOf(element));
            log.info("Waited for element " + elementName + " " + elementType + " to be visible");
            nlpResponseModel.setMessage(modifiedPassMessage);
            nlpResponseModel.setStatus(CommonConstants.pass);
        } catch (Exception exception) {
            log.error("NLP_EXCEPTION in WaitTillElementVisible ", exception);
