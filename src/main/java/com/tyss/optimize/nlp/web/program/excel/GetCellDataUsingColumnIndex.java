...
@Slf4j
@Component(value =  "GetCellDataUsingColumnIndex")
public class GetCellDataUsingColumnIndex implements Nlp {
    @Override
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        Integer rowIndex = null, columnIndex = null;
        String cellValue = "";
        try {
            ...
            Cell cell = sh.getRow(rowIndex).getCell(columnIndex, MissingCellPolicy.CREATE_NULL_AS_BLANK);
            cellValue = cell.toString();
            log.info("Cell data present at row "+rowIndex+" column "+columnIndex+" is "+cellValue);
            nlpResponseModel.setMessage(passMessage.replace("*rowIndex*",rowIndex.toString()).replace("*columnIndex*",columnIndex.toString()).replace("*returnValue*",cellValue));
            nlpResponseModel.setStatus(CommonConstants.pass);
            nlpResponseModel.getAttributes().put("cellValue", cellValue);
        } catch (Exception exception) {
            log.error("NLP_EXCEPTION in GetCellDataUsingColumnIndex ", exception);
            failMessage = failMessage.replace("*rowIndex*", rowIndex.toString()).replace("*columnIndex*", columnIndex.toString());
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("	Cell cell = sh.getRow(rowIndex).getCell(columnIndex, MissingCellPolicy.CREATE_NULL_AS_BLANK);\n");
        sb.append("	cellValue = cell.toString();\n");
