...
@Slf4j
@Component(value = "VerifyIfGivenColumnHeaderIsDuplicate")
public class VerifyIfGivenColumnHeaderIsDuplicate implements Nlp {
    @Override
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            if (count > 1) {
                log.info("Successfully verified column " + columnHeader + " is duplicate in sheet " + sheetName + " and workbook path " + filePath);
                ...
                log.error("Failed to verify if given column " + columnHeader + " is duplicate in sheet " + sheetName + " and workbook file path " + filePath);
                nlpResponseModel.setMessage(modifiedFailMessage);
                nlpResponseModel.setStatus(CommonConstants.fail);
                nlpResponseModel.setIfCheckPointIsFailed(ifCheckPointIsFailed);
            }
        } catch (Exception exception) {
            log.error("NLP_EXCEPTION in VerifyIfGivenColumnHeaderIsDuplicate ", exception);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("		System.out.println(\"Successfully verified column \" + columnHeader + \" is duplicate in sheet \" + sheetName + \" and workbook path \" + filePath);\n");
        sb.append("	} else {\n");
        sb.append("		System.out.println(\"Failed to verify if given column \" + columnHeader + \" is duplicate in sheet \" + sheetName + \" and workbook file path \" + filePath);\n");
        sb.append("	}\n");
