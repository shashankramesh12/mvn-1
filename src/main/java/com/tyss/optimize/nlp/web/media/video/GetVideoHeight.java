...
@Slf4j
@Component(value = "GetVideoHeight")
public class GetVideoHeight implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            Long height = (Long) ((JavascriptExecutor) driver).executeScript("return arguments[0].videoHeight;", element);
            passMessage = passMessage.replace("*returnValue*", height.toString());
            nlpResponseModel.getAttributes().put("videoHeight", (double)height);
            nlpResponseModel.setStatus(CommonConstants.pass);
            nlpResponseModel.setMessage(passMessage);
        } catch (Exception exception){
            log.error("NLP_EXCEPTION in GetVideoHeight ", exception);
