...
@Slf4j
@Component(value = "DeselectByValue")
public class DeselectByValue implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            log.info("Deselecting option from " + elementName + " " + elementType + " whose value is " + value);
            String modifiedPassMessage = passMessage.replace("*elementName*", elementName)
                    .replace("*elementType*", elementType).replace("*value*", value);
            modifiedFailMessage = failMessage.replace("*elementName*", elementName)
                    .replace("*elementType*", elementType).replace("*value*", value);
            Select select = new Select(element);
            select.deselectByValue(value);
            log.info("Deslected option from " + elementName + " " + elementType + " whose value is " + value);
            nlpResponseModel.setMessage(modifiedPassMessage);
            nlpResponseModel.setStatus(CommonConstants.pass);
        } catch (Throwable e) {
            log.error("NLP_EXCEPTION in DeselectByValue ", e);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("select.deselectByValue(value);\n");
