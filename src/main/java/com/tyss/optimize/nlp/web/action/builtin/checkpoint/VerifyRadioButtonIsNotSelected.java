...
@Slf4j
@Component(value = "VerifyRadioButtonIsNotSelected")
public class VerifyRadioButtonIsNotSelected implements Nlp {
		...
		try {
            ...
            if (!(tag.equalsIgnoreCase("input") && (type.equalsIgnoreCase("radio")))) {
                ...
                if (actualSelectedStatus) {
                    ...
                    log.info(elementName + " " + elementType + " RadioButton is not selected");
            ...
            log.error("NLP_EXCEPTION in VerifyRadioButtonIsNotSelected ", exception);
