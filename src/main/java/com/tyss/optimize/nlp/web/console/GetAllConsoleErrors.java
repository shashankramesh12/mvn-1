...
@Slf4j
@Component(value =  "GetAllConsoleErrors")
public class GetAllConsoleErrors implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            if(logs.size() > 0){
                logs.stream().filter(log -> log.getLevel().toString().equalsIgnoreCase("SEVERE")).map(log -> logEntries.add(log.getMessage())).collect(Collectors.toList());
            }
            if(logEntries.isEmpty()){
                ...
                new WebDriverDevTools().getDevTools(driver).addListener(Log.entryAdded(),logEntry -> {
                    if(logEntry.getLevel().name().equalsIgnoreCase("ERROR")){
            ...
            log.error("NLP_EXCEPTION in GetAllConsoleErrors ", exception);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("	logs.stream().filter(log -> log.getLevel().toString().equalsIgnoreCase(\"SEVERE\")).map(log -> logEntries.add(log.getMessage())).collect(Collectors.toList());\n");
        sb.append("}\n");
        ...
        sb.append("		if(logEntry.getLevel().name().equalsIgnoreCase(\"ERROR\")){\n");
