...
@Slf4j
@Component(value = "FilterApiBasedOnSearchKey")
public class FilterApiBasedOnSearchKey implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            String searchKey = attributes.get("searchKey").toString();
            modifiedFailMessage = modifiedFailMessage.replace("*searchKey*", searchKey);
            passMessage = passMessage.replace("*searchKey*", searchKey);
            ...
            new WebDriverDevTools().getDevTools(driver).addListener(Network.responseReceived(), responseReceived -> {
                if(responseReceived.getResponse().getUrl().contains(searchKey)) {
            ...
            log.error("NLP_EXCEPTION in FilterApiBasedOnSearchKey ", exception);
