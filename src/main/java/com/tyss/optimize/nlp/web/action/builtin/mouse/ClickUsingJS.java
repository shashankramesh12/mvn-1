...
@Slf4j
@Component(value = "ClickUsingJS")
public class ClickUsingJS implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            modifiedFailMessage = failMessage.replace("*elementName*", elementName)
                    .replace("*elementType*", elementType);
            log.info("Clicking on " + elementName + " " + elementType + " by javascript executor");
            ((JavascriptExecutor) driver).executeScript("arguments[0].click()", element);
            log.info("Clicked on " + elementName + " " + elementType + " by javascript executor");
            nlpResponseModel.setMessage(modifiedPassMessage);
            nlpResponseModel.setStatus(CommonConstants.pass);
        } catch (Throwable e) {
            log.error("NLP_EXCEPTION in ClickUsingJS ", e);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("((JavascriptExecutor) driver).executeScript(\"arguments[0].click()\", element);\n");
