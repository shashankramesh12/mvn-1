...
@Slf4j
@Component(value = "DeselectAll")
public class DeselectAll implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            modifiedFailMessage = failMessage.replace("*elementName*", elementName)
                    .replace("*elementType*", elementType);
            log.info("Deselecting all the options from " + elementName + " " + elementType);

            Select select = new Select(element);
            select.deselectAll();
            log.info("Deselected all options from " + elementName + " " + elementType);
            nlpResponseModel.setMessage(modifiedPassMessage);
            nlpResponseModel.setStatus(CommonConstants.pass);
        } catch (Throwable exception) {
            log.error("NLP_EXCEPTION in DeselectAll ", exception);
