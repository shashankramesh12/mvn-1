...
@Slf4j
@Component(value = "DeSelectTheCheckBox")
public class DeSelectTheCheckBox implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            log.info("Checkbox deselecting " + elementName + " " + elementType);
            if (!(tag.equalsIgnoreCase("input") && type.equalsIgnoreCase("checkbox"))) {
                ...
                if (selected) {
                    log.info("Check Box is already selected;so deselecting it");
                    new Click().execute(requestClick);
                    nlpResponseModel.setMessage(modifiedPassMessage);
                    nlpResponseModel.setStatus(CommonConstants.pass);
                } else {
                    log.info("Check Box is already deselected");
                    nlpResponseModel.setMessage(modifiedFailMessage);
                    nlpResponseModel.setStatus(CommonConstants.fail);
                    nlpResponseModel.setIfCheckPointIsFailed(ifCheckPointIsFailed);
                }
                log.info("Deselected " + elementName + " " + elementType);
            }
            ...
            log.error("NLP_EXCEPTION in DeSelectTheCheckBox ", exception);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("		System.out.println(\"Check Box is already selected;so deselecting it\");\n");
        sb.append("		element.click();\n");
        sb.append("	} else {\n");
        sb.append("		System.out.println(\"Check Box is already deselected\");\n");
        sb.append("	}\n");
        sb.append("}\n");
