...
@Slf4j
@Component(value = "GetAllConsoleInformation")
public class GetAllConsoleInformation implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            if(logs.size() > 0){
                logs.stream().filter(log -> log.getLevel().toString().equalsIgnoreCase("INFO")).map(log -> logEntries.add(log.getMessage())).collect(Collectors.toList());
            }
            if(logEntries.isEmpty()){
                ...
                new WebDriverDevTools().getDevTools(driver).addListener(Log.entryAdded(), logEntry -> {
                    if(logEntry.getLevel().name().equalsIgnoreCase("INFO")){
            ...
            log.error("NLP_EXCEPTION in GetAllConsoleInformation ", exception);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("	logs.stream().filter(log -> log.getLevel().toString().equalsIgnoreCase(\"INFO\")).map(log -> logEntries.add(log.getMessage())).collect(Collectors.toList());\n");
        sb.append("}\n");
        ...
        sb.append("		if(logEntry.getLevel().name().equalsIgnoreCase(\"INFO\")){\n");
