...
@Slf4j
@Component(value = "GetFirstSelectedOption")
public class GetFirstSelectedOption implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            log.info("Getting the first selected option from the " + elementName + " " + elementType);
            select = new Select(element);
            firstElement = select.getFirstSelectedOption();
            log.info("Captured first selected option from the " + elementName + " " + elementType);
            nlpResponseModel.getAttributes().put("firstElement", firstElement);
            nlpResponseModel.setMessage(modifiedPassMessage);
            nlpResponseModel.setStatus(CommonConstants.pass);
        } catch (Throwable e) {
            log.error("NLP_EXCEPTION in GetFirstSelectedOption ", e);
