...
@Slf4j
@Component(value = "DragAndDropToLocation")
public class DragAndDropToLocation implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            String modifiedPassMessage = passMessage.replace("*elementName*", elementName)
                    .replace("*elementType*", elementType).replace("*xOffset*", String.valueOf(xOffset))
                    .replace("*yOffset*", String.valueOf(yOffset));
            modifiedFailMessage = failMessage.replace("*elementName*", elementName)
                    .replace("*elementType*", elementType).replace("*xOffset*", String.valueOf(xOffset))
                    .replace("*yOffset*", String.valueOf(yOffset));
            log.info("Dragging the element " + elementName + " " + elementType
                    + " and dropping the element to specific coordinates:- x:" + xOffset + " ; y:" + yOffset);
            Actions actions = new Actions(driver);
            actions.dragAndDropBy(element, xOffset, yOffset).perform();
            nlpResponseModel.setMessage(modifiedPassMessage);
            nlpResponseModel.setStatus(CommonConstants.pass);
        } catch (Throwable e) {
            log.error("NLP_EXCEPTION in DragAndDropToLocation ", e);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("actions.dragAndDropBy(element, xOffset, yOffset).perform();\n");
