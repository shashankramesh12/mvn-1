...
@Slf4j
@Component(value = "GetSingleApiStatusCode")
public class GetSingleApiStatusCode implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            passMessage = passMessage.replace("*returnValue*", response.get().getStatus().toString());
            nlpResponseModel.getAttributes().put("returnValue", response.get().getStatus());
            nlpResponseModel.setStatus(CommonConstants.pass);
            nlpResponseModel.setMessage(passMessage);
        } catch (Exception exception){
            log.error("NLP_EXCEPTION in GetSingleApiStatusCode ", exception);
