...
@Slf4j
@Component(value = "GetVideoCurrentSeekTime")
public class GetVideoCurrentSeekTime implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            Double currentTime = (Double) ((JavascriptExecutor) driver).executeScript("return arguments[0].currentTime;", element);
            passMessage = passMessage.replace("*returnValue*",currentTime.toString());
            nlpResponseModel.getAttributes().put("currentSeekTime", currentTime);
            nlpResponseModel.setStatus(CommonConstants.pass);
            nlpResponseModel.setMessage(passMessage);
        } catch (Exception exception){
            log.error("NLP_EXCEPTION in GetVideoCurrentSeekTime ", exception);
