...
@Slf4j
@Component(value = "GetVideoWidth")
public class GetVideoWidth implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            Long width = (Long) ((JavascriptExecutor) driver).executeScript("return arguments[0].videoWidth;", element);
            passMessage = passMessage.replace("*returnValue*", width.toString());
            nlpResponseModel.getAttributes().put("videoWidth", (double)width);
            nlpResponseModel.setStatus(CommonConstants.pass);
            nlpResponseModel.setMessage(passMessage);
        } catch (Exception exception){
            log.error("NLP_EXCEPTION in GetVideoWidth ", exception);
