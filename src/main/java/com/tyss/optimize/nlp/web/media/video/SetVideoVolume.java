...
@Slf4j
@Component(value = "SetVideoVolume")
public class SetVideoVolume implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            ((JavascriptExecutor) driver).executeScript("arguments[0].volume=" + volume + ";", element);
            Double currentVolume = (Double) ((JavascriptExecutor) driver).executeScript("return arguments[0].volume;", element);
            passMessage = passMessage.replace("*returnValue*", currentVolume.toString());
            nlpResponseModel.getAttributes().put("volume", currentVolume);
            nlpResponseModel.setStatus(CommonConstants.pass);
            nlpResponseModel.setMessage(passMessage);
        } catch (Exception exception){
            log.error("NLP_EXCEPTION in SetVideoVolume ", exception);
