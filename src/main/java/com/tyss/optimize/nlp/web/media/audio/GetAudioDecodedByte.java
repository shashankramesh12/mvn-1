...
@Slf4j
@Component(value = "GetAudioDecodedByte")
public class GetAudioDecodedByte implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try{
            ...
            Long decodedByte = (Long) ((JavascriptExecutor) driver).executeScript("return arguments[0].webkitAudioDecodedByteCount;", element);
            passMessage = passMessage.replace("*returnValue*", decodedByte.toString());
            nlpResponseModel.getAttributes().put("decodedByte", decodedByte);
            nlpResponseModel.setStatus(CommonConstants.pass);
            nlpResponseModel.setMessage(passMessage);
        } catch (Exception exception){
            log.error("NLP_EXCEPTION in GetAudioDecodedByte ", exception);
