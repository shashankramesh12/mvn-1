...
@Slf4j
@Component(value = "GetAllTheSelectedOptionsFromListBoxAsText")
public class GetAllTheSelectedOptionsFromListBoxAsText implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            log.info("Capturing all selected options from " + elementName + " " + elementType);
            ...
            if (status) {
                List<WebElement> allOptions = (List<WebElement>) ((NlpResponseModel) new GetAllSelectedOptions().execute(requestAllSelected))
                        .getAttributes().get("elementList");
                requestText.getAttributes().put("allElements", allOptions);
                log.info("Total Number of Selected Options:" + allOptions.size());
                ...
                log.info("Captured all selected options from " + elementName + " " + elementType);
            } else {
                log.error("This action cannot be performed on NON-ListBox element");
                log.error("Failed to get all selected options from " + elementName + " " + elementType);
            ...
            log.error("NLP_EXCEPTION in GetAllTheSelectedOptionsFromListBoxAsText ", exception);
        ...
        String selectedText = strBuilder.toString();
        nlpResponseModel.getAttributes().put("selectedText", selectedText);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("	elementList = select.getAllSelectedOptions();\n");
        ...
        sb.append("	}\n");
        sb.append("	String selectedText = strBuilder.toString();\n");
        sb.append("}else{\n");
