...
@Slf4j
@Component(value = "GetHeightOfWebElement")
public class GetHeightOfWebElement implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            log.info("Fetching height of " + elementName + " " + elementType);
            height = element.getSize().getHeight();
            log.info("Successfully fetched height of " + elementName + " " + elementType);
            nlpResponseModel.setMessage(modifiedPassMessage);
            nlpResponseModel.setStatus(CommonConstants.pass);
            nlpResponseModel.getAttributes().put("height", height);
        } catch (Exception exception) {
            log.error("NLP_EXCEPTION in GetHeight of Webelement ", exception);
