...
@Slf4j
@Component(value = "GetYLocationOfWebElement")
public class GetYLocationOfWebElement implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            log.info("Fetching yCoordinate of " + elementName + " " + elementType);
            y = element.getLocation().getY();
            log.info("Successfully fetched yCoordinate of " + elementName + " " + elementType);
            nlpResponseModel.setMessage(modifiedPassMessage);
            nlpResponseModel.setStatus(CommonConstants.pass);
            nlpResponseModel.getAttributes().put("y", y);
        } catch (Exception exception) {
            log.error("NLP_EXCEPTION in GetYLocationOfWebElement ", exception);
