...
@Slf4j
@Component(value = "GetIndexOfColumnIfGivenDataPresent")
public class GetIndexOfColumnIfGivenDataPresent implements Nlp {
    @Override
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            for (int i = 1; i < rowCount; i++) {
                ...
                if (flag == true) {
                    log.info("Index of column with data " + data + " is " + columnIndex);
                    nlpResponseModel.setMessage(passMessage.replace("*data*", data).replace("*returnValue*", columnIndex.toString()));
                    nlpResponseModel.setStatus(CommonConstants.pass);
                    break;
                }
            }
            if (flag == false) {
                log.error("Failed to fetch index of column where data is " + data);
            ...
            log.error("NLP_EXCEPTION in GetIndexofColumnIfGivenDataPresent ", exception);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("			System.out.println(\"Index of column with Data \" + data + \" is \" + columnIndex);\n");
        sb.append("			break;\n");
        sb.append("		}\n");
        sb.append("	}\n");
        sb.append("	if (flag == false) {\n");
        sb.append("		System.out.println(\"Failed to fetch index of column where Data is \" + data);\n");
        sb.append("	}\n");
