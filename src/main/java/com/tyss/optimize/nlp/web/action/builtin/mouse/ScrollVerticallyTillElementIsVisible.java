...
@Slf4j
@Component(value = "ScrollVerticallyTillElementIsVisible")
public class ScrollVerticallyTillElementIsVisible implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            log.info("Browser window scrolling vertically till " + elementName + " " + elementType + " is visible on page");
            ...
            modifiedFailMessage = failMessage.replace("*elementName*", elementName)
                    .replace("*elementType*", elementType);
            log.info("Window scrolled vertically till " + elementName + " " + elementType + " is visible on page");
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView()", element);
            nlpResponseModel.setMessage(modifiedPassMessage);
            nlpResponseModel.setStatus(CommonConstants.pass);
        } catch (Exception exception) {
            log.error("NLP_EXCEPTION in ScrollVerticallyTillElementIsVisible ", exception);
