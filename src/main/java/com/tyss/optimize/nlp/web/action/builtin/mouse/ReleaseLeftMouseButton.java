...
@Slf4j
@Component(value = "ReleaseLeftMouseButton")
public class ReleaseLeftMouseButton implements Nlp {
    ...
    public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
        ...
        try {
            ...
            modifiedFailMessage = failMessage.replace("*elementName*", elementName)
                    .replace("*elementType*", elementType);
            log.info("Releasing the pressed left mouse button on " + elementName + " " + elementType);
            Actions actions = new Actions(driver);
            actions.release(element).build().perform();
            nlpResponseModel.setMessage(modifiedPassMessage);
            nlpResponseModel.setStatus(CommonConstants.pass);
        } catch (Throwable e) {
            log.error("NLP_EXCEPTION in ReleaseLeftMouseButton ", e);
    ...
    public StringBuilder getTestCode() throws NlpException {
        ...
        sb.append("actions.release(element).build().perform();\n");
